...
ver=str(rho)+"_"+str(ave_flow)+"_"+str(static_dia)+"_"+str(v0)+"_"+str(DR)
...
input_traj = gsd.hoomd.open('../../data_file/ver4/{0}/log_pos_{0}.gsd'.format(init_ver), 'rb')
...
ljw.params['Move'] = {"epsilon": epsilon_wall, "sigma": sigma_aa/2.0, "r_cut": sigma_ab*2**(1/6)-static_dia/2}
...
#???????
odb = hoomd.md.methods.OverdampedViscous(filter=hoomd.filter.All())
odb.gamma.default = kbT / DTrans
# ここで指定したgamma_rはアクティブ力による回転には関係ないから、defultのままで指定しなくてもいい
# 混乱を避けるために一応コメントアウトしておく
# odb.gamma_r.default =[ktemp / rotational_diffusion,ktemp / rotational_diffusion,0]
...
flow_force=ave_flow* odb.gamma.default
...
integrator = hoomd.md.Integrator(
 dt=dt,
 methods=[odb],
 forces=[lj,constant, ljw,active,random_force],
)
...
gsd_writer_pos = hoomd.write.GSD(filename=media_path+"/log_pos.gsd",
...
# ファイルに保存.pickle
with open(media_path+'/data.pickle', 'wb') as handle:
...
os.chdir("../")
traj = gsd.hoomd.open(media_path+"/log_pos.gsd", 'rb')
