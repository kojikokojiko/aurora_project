...
vector<double> rx, vector<double> ry,int NP,vector<double> diameter){
    ...
    double rc,rc2;
    double fx,fy;
    double unit_vector_x,unit_vector_y;
    ...
    for (int i=0;i<NP;i++){
        if (rx[i]<0.0){
            ...
            temp_r2=temp_rxij*temp_rxij+temp_ryij*temp_ryij;
            temp_r=sqrt(temp_r2);
            // 力の方向の単位ベクトル
            unit_vector_x=temp_rxij/temp_r;
            unit_vector_y=temp_ryij/temp_r;
            // 円柱表面から粒子までの距離
            r=temp_r-STATIC_DIA/2.0;
            rxij=r*unit_vector_x;
            ryij=r*unit_vector_y;
